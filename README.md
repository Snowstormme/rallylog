...
1. Create a Neon project in a nearby region and add a role named `rallylog_app`.
2. Copy the owner TLS connection string into the GitHub repository secret `DATABASE_OWNER_URL`, then run **Initialize production database**. The action creates the schema, loads the bundled catalog of 19,903 ATP/WTA matches, removes elevated privileges from `rallylog_app`, grants only runtime data access and verifies the result. Rotate the owner password and remove the temporary GitHub secret afterward.
3. Copy the pooled TLS URL for `rallylog_app` into Render's `DATABASE_URL`.
